...
def test_s3_storage_upload(mock_boto_client):
    ...
    storage = S3Storage(bucket="test-bucket", default_acl="private")
...
def test_s3_storage_upload_with_prefix(mock_boto_client):
    ...
    storage = S3Storage(bucket="test-bucket", prefix="dbback", default_acl="private")
...
def test_s3_storage_download_relative(mock_boto_client):
    ...
    mock_s3.download_file.assert_called_once_with("test-bucket", "dbback/remote.sql", str(local_path))
...
def test_s3_storage_upload_fileobj(mock_boto_client):
    ...
    mock_s3.upload_fileobj.assert_called_once_with(
        fileobj, "test-bucket", "dbback/test.sql", ExtraArgs={"ACL": "private"}
    )
